<template>
  <div class="search-movie-wrap">
    <input v-model="searchWord" ref="zSearch" class="search-movie-input" id="searchInput" type="text" @click="handleClick" @focus="handleFocus" @blur="handleBlur" @input="handleInput" @keyup.enter="handleEnter" @keyup.up="handleKeyupUp" @keyup.down="handleKeyupDown" autocomplete="off" />
    ...
    <div class="search-movie-suggest" v-show="isShowOptions" ref="zSearchOptions">
      <router-link v-for="(item, index) in suggestList" :key="index" :to="{ path: `/movie/detail/${item.id}`, target: '_blank' }" :class="`search-movie-suggest-item ${cursorIndex === index ? 'search-movie-suggest-item-active' : ''}`" target="_blank" @mouseover="handleMouseoverOptionItem(index)">
